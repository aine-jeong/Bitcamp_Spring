...
@Repository //전제조건: 'NewNoticeDao를 자동으로 Bean 생성시키겠다'
public class NewNoticeDao implements NoticeDao {
	...
	public int getCount(String field, String query) throws ClassNotFoundException, SQLException {
		...
		//파라미터) 1: 실행될 쿼리문 / 2: 리턴타입에 대한 타입 정보 / 3: 파라미터 값 설정 (?에 들어가는 값)
	...
	public List<Notice> getNotices(int page, String field, String query) throws ClassNotFoundException, SQLException {
		...
		String sql = "SELECT * FROM"; // select절에 * 쓰지 마세용!!! (안좋은 습관)
		...
		//jdbctemplate.query
		//파라미터) 1. 실행될 쿼리문 / 2: ?에 들어가는 파라미터를 배열로 담아주기 / 3: 자동매핑
		//BeanPropertyRowMapper(객체 속성, 데이터베이스에서 셀렉트한 row 를 연결)
		//DB에서 select한 결과와, 만들어둔 DTO를 연결시켜주겠다(자동매핑)
		//Generic으로 타입을 주면 -> 해당 Generic타입으로 객체 만들어서 데이터 건수만큼의 결과를 list로 추가해준다.
	...
	public Notice getNotice(String seq) throws ClassNotFoundException, SQLException {
		...
		//자동 매핑이 되지 않는 경우 : 셀렉트한 결과 컬럼명과 dto의 멤버필드명이 하나라도 일치하지 않으면 Mapping되지 않는다.
		//아래와 같은 작업을 할거면 Template을 사용할 이유가 없다..(아래는 잘못된 사례다 ㅠ_ㅠ)
		//자동화에 대한 준비를 철저하게 하자 -> UI (input name="") == dto (member field) == DB(컬럼명)
		//							ㄴ> 반드시 같은 이름으로 만들어두기
