...
public class EmpController {
	...
	// dao
	/*
	 * GET ( /empdata/100 ) >> 100번 게시물 조회 POST ( /empdata + 데이터(json)) >> 사원등록
	 * DELETE ( /empdata/100) >> 100게시물 삭제 PUT ( /empdata/100 + 데이터 >> 100번 게시물 수정
	 */
	...
	public ResponseEntity<String> register(@PathVariable("no") int no, @RequestBody Emp emp) {
		try {
			...
			return new ResponseEntity<String>("emp data update success", HttpStatus.OK); // 200
		} catch (Exception e) {
			return new ResponseEntity<String>(e.getMessage(), HttpStatus.BAD_REQUEST); // 400
		}
	}
